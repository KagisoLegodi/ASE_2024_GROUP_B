 ...
 */
export async function fetchRecipes(page, limit, search, category, tags, steps, ingredients = [], topRated = false, ingredientsMatchType) {
  try {
    // Construct the URL
...
export async function fetchIngredients() {
  try {
    const url = `${process.env.NEXT_PUBLIC_BASE_URL || process.env.BASE_URL}/api/Ingredients`;
    const response = await fetch(url);
