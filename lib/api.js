...
/* HERE YOU NOW NEED TO MAKE SURE THAT ONLY THE RECIPES THAT MATCH THE ÌNCOMING RECIPEID'S ARE FETCHED AND RETURNED */
export async function fetchFavourites(favouritesNumbers, page, limit, search, category, tags, steps, ingredients = [], topRated = false, ingredientsMatchType) {
  try {
    // Construct the URL
    const url = new URL(`${process.env.NEXT_PUBLIC_BASE_URL}/api/recipes`);
    // Append query parameters only if they are defined
    ...
    console.log("url:", url); // Log the constructed URL
    const response = await fetch(url); // HERE ONLY THE RECIPES WITH THE INCOMING NUMBERS SHOULD BE FETCHED
...
/**
 * Adds a recipe to user's favourites.
 * 
 * @async
 * @function addToFavourites
 * @param {string} token - JWT token for authentication
 * @param {string} recipeId - ID of the recipe to add to favourites
 * @returns {Promise<Object>} - A promise that resolves to the response message
 */
export async function addToFavourites(token, recipeId) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/favourites`, {
      method: 'POST',
      headers: {
    ...
    return { error: `Error adding to favourites: ${error.message}` };
  }
}

/**
 * Removes a recipe from user's favourites.
 * 
 * @async
 * @function removeFromFavourites
 * @param {string} token - JWT token for authentication
 * @param {string} recipeId - ID of the recipe to remove from favourites
 * @returns {Promise<Object>} - A promise that resolves to the response message
 */
export async function removeFromFavourites(token, recipeId) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/favourites`, {
      method: 'DELETE',
      headers: {
    ...
    return { error: `Error removing from favourites: ${error.message}` };
  }
}