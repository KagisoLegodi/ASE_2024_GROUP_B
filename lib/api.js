 ...
 */
export async function fetchRecipes(page, limit, search, category) {
  try {
    // Construct the URL
...
/**
 * Fetches recipe suggestions based on a search term from the '/api/recipes/suggestions' endpoint.
 *
 * This function sends a GET request with the search term to retrieve relevant recipe suggestions.
 * If the request fails, it returns an error message.
 *
 * @async
 * @function getRecipeSuggestions
 * @param {string} searchTerm - The term to search for recipe suggestions.
 * @returns {Promise<Object>} - A promise that resolves to an array of recipe suggestion objects if successful, or an error message if failed.
 */
export async function getRecipeSuggestions(searchTerm) {
  try {
    // Construct the URL with the search term
    const url = new URL(`${process.env.NEXT_PUBLIC_BASE_URL}/api/recipes/suggestions`);
    ...
    return { error: `Error fetching recipe suggestions: ${error.message}` };
  }
}
